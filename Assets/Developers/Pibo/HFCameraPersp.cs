...
public class HFCameraPersp : MonoBehaviour
{
	...
	[SerializeField]
	private Vector3 m_cameraStartPosition = new Vector3(0f, 0f, -30f);
	...
	[SerializeField]
	float m_minZoomSize = 8f;

	[Range(16f, 64f)]
	[Tooltip("Maximum orthographic size")]
	[SerializeField]
	float m_maxZoomSize = 32f;
	...
	[SerializeField]
	float m_minPitchAngle = 40f;

	[Range(45f, 89f)]
	[Tooltip("Maximum pitch angle")]
	[SerializeField]
	float m_maxPitchAngle = 80f;
	...
	private Vector3 HandleScrollInput()
	{
		...
		float boost = (m_camera.fieldOfView - m_minZoomSize) / (m_maxZoomSize - m_minZoomSize) * m_scrollBoost;
	...
	private void Zoom()
	{
		...
		if (HandleZoomInput() != 0f)
		{
			...
			float damp = (m_maxZoomSize - m_camera.fieldOfView) / (m_maxZoomSize - m_minZoomSize) - 0.5f;
			// Apply input
			float targetSize = m_camera.fieldOfView - HandleZoomInput() * Time.deltaTime * (1 - damp);
			// Clamp
			m_targetZoom = Mathf.Clamp(targetSize, m_minZoomSize, m_maxZoomSize); 
		}
		
		m_camera.fieldOfView = Mathf.Lerp(m_camera.fieldOfView, m_targetZoom, m_zoomLerp);
	}
	...
	private float HandleZoomInput()
	{
		...
		// Mouse
		if (Input.GetAxis("WheelZoom") != 0f)//@TODO && !UIManager.Instance.IsMouseOverUI())
		{

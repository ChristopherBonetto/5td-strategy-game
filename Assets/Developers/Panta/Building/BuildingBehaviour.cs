...
namespace HF.Unit
{
    ...
    public class BuildingBehaviour : EntityBehavior
    {
        ...
        public void CarryBuilding(Vector3 inCarryPosition, out bool success)
        {
            if (m_buildingStats.BuildingType == Types.BuildingType.CASTLE)
            {
        ...
        public void DropBuilding(Vector3 inDropPosition, out bool success)
        {
        ...
        private void DetectArea()
        {
            if (m_timeElapsedbetweenDetections > m_delayBetweenDetections)
            {
                ...
                if (m_ostileRef == null)
                {
                    ...
                    m_ostileRef = m_detectionArea.DetectFaction(transform, m_buildingStats.AttackRange, m_ostileLayer, gameObject.layer);
        ...
        public override void Attack()
        {
            if (m_ostileRef != null)
            {
                ...
                // then deal damage after the fixed time.

                // after shoot check if the target is still in range.
                if (Vector3.Distance(transform.position, m_ostileRef.transform.position) > m_buildingStats.AttackRange)
                {
                    m_ostileRef = null;
                }
            }
